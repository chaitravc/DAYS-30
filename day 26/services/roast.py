...
def categorize_roast_topic(user_query: str) -> str:
    ...
    if any(word in query_lower for word in ["procrastinate", "lazy", "delay", "later", "tomorrow"]):
        return "procrastination"
    elif any(word in query_lower for word in ["decision", "choice", "mistake", "stupid", "dumb"]):
        return "bad_decisions"
    elif any(word in query_lower for word in ["computer", "phone", "app", "tech", "wifi", "internet"]):
        return "technology"
    elif any(word in query_lower for word in ["work", "job", "boss", "meeting", "office"]):
        return "work"
    elif any(word in query_lower for word in ["life", "relationship", "friend", "family"]):
        return "lifestyle"
    else:
        return "generic"
...
def generate_roast(roast_info: Dict[str, Any]) -> str:
    ...
    if roast_type == "self_roast":
        ...
        category = categorize_roast_topic(roast_info["context"])
